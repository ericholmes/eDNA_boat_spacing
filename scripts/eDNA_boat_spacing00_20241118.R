...
stryker_startstop <- data.frame(boat = rep("stryker",7), 
                        ...
                        start = c(7, 43, 94, 131, 201, 297, 376), 
                        end = c(39, 90, 127, 197, 293, 372, 423))
...
## New Alosa ----
new_alosa_startstop <- data.frame(boat = rep("new_alosa", 4), 
                                site = c(rep("812", 1), rep("809", 3)),
                                start = c(7, 309, 443, 487), 
                                end = c(305, 439, 483, 554))
...
for(xs in 1:nrow(new_alosa_startstop)){
  ...
  temp <- read.csv("data/Survey00_20241119/RouteHistoryFullBkup_CDFW.csv", 
...
png("output/eDNA_vessel_spacing_20241119_%02d.png",
...
png("output/eDNA_vessel_velocity_20241119_%02d.png",
